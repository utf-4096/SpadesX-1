...
#define VSID VXL_MAX_SIZE
#define VXL_MAX_SIZEM (VXL_MAX_SIZE - 1)
#define VSIDM VXL_MAX_SIZEM
...
unsigned int seed = 0;
static inline void         set_seed(unsigned int value)
{
    seed = value;
}
...
static inline int get_height_pos(int x, int y)
{
    return y * VSID + x;
}

static inline int is_valid_position(mapvxl_t* map, int x, int y, int z)
{
    return x >= 0 && x < map->size_x && y >= 0 && y < map->size_y && z >= 0 && z < map->size_z;
}

static inline int get_height(mapvxl_t* map, int x, int y, int def)
{
    if (!is_valid_position(map, x, y, 0))
        return def;
    return buf[get_height_pos(x, y)].a;
}

static inline int get_lowest_height(mapvxl_t* map, int x, int y)
{
    // clang-format off
    int z = get_height(map, x, y, 63);
    z = max(get_height(map, x - 1, y, z),
        max(get_height(map, x + 1, y, z),
        max(get_height(map, x, y - 1, z),
        max(get_height(map, x, y + 1, z),
            z))));
    return z;
    // clang-format on
...
void genland(classicgen_opt_t options, mapvxl_t* map)
{
    ...
    for (y = 0, k = 0; y < VSID; y++) {
        for (x = 0; x < VSID; x++, k++) {
            height = buf[k].a;
            for (z = 63; z > height; z--) {
                mapvxl_set_color(map, x, y, z, MAPVXL_DEFAULT_COLOR);
            }
            mapvxl_set_color(map, x, y, z, MAPVXL_DEFAULT_COLOR);
            lowest_z                        = get_lowest_height(map, x, y) + 1;

            for (; z < lowest_z; z++) {
                mapvxl_set_color(map, x, y, z, ((int *)&buf[k])[0]);
            }
        }
    }
